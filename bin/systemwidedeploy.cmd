...
@IF %botmod% EQU 1 set deploychoice=%1
@call modules\prompt.cmd deploychoice "Enter choice:"
...
@if "%deploychoice%"=="7" IF %mesainstalled% EQU 0 IF %botmod% EQU 1 set deployerror=Unattended mode does not support exit command.
@if "%deploychoice%"=="8" IF %mesainstalled% EQU 0 set deployerror=Error^: No Mesa3D drivers installed.
@if "%deploychoice%"=="9" IF %mesainstalled% GTR 0 IF %botmod% EQU 1 set deployerror=Unattended mode does not support exit command.
...
@IF %botmod% EQU 1 set keepdxil=n
